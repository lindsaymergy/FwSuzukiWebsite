...
!function(app){"use strict";app.registerModule("activities")}(ApplicationConfiguration);
...
!function(app){"use strict";app.registerModule("students")}(ApplicationConfiguration);
...
!function(){"use strict";function menuConfig(Menus){Menus.addMenuItem("topbar",{title:"Students",state:"students",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","students",{title:"List Students",state:"students.list"}),Menus.addSubMenuItem("topbar","students",{title:"Create Student",state:"students.create",roles:["user"]})}angular.module("students").run(menuConfig),menuConfig.$inject=["Menus"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("students",{"abstract":!0,url:"/students",template:"<ui-view/>"}).state("students.list",{url:"",templateUrl:"modules/students/client/views/list-students.client.view.html",controller:"StudentsListController",controllerAs:"vm",data:{pageTitle:"Students List"}}).state("students.create",{url:"/create",templateUrl:"modules/students/client/views/form-student.client.view.html",controller:"StudentsController",controllerAs:"vm",resolve:{studentResolve:newStudent},data:{roles:["user","admin"],pageTitle:"Students Create"}}).state("students.edit",{url:"/:studentId/edit",templateUrl:"modules/students/client/views/form-student.client.view.html",controller:"StudentsController",controllerAs:"vm",resolve:{studentResolve:getStudent},data:{roles:["user","admin"],pageTitle:"Edit Student {{ studentResolve.name }}"}}).state("students.view",{url:"/:studentId",templateUrl:"modules/students/client/views/view-student.client.view.html",controller:"StudentsController",controllerAs:"vm",resolve:{studentResolve:getStudent},data:{pageTitle:"Student {{ studentResolve.name }}"}})}function getStudent($stateParams,StudentsService){return StudentsService.get({studentId:$stateParams.studentId}).$promise}function newStudent(StudentsService){return new StudentsService}angular.module("students").config(routeConfig),routeConfig.$inject=["$stateProvider"],getStudent.$inject=["$stateParams","StudentsService"],newStudent.$inject=["StudentsService"]}();
!function(){"use strict";function StudentsListController(StudentsService){var vm=this;vm.students=StudentsService.query()}angular.module("students").controller("StudentsListController",StudentsListController),StudentsListController.$inject=["StudentsService"]}();
!function(){"use strict";function StudentsController($scope,$state,$window,Authentication,student){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.student.$remove($state.go("students.list"))}function save(isValid){function successCallback(res){$state.go("students.view",{studentId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.student._id?vm.student.$update(successCallback,errorCallback):vm.student.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.studentForm"),!1)}var vm=this;vm.authentication=Authentication,vm.student=student,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("students").controller("StudentsController",StudentsController),StudentsController.$inject=["$scope","$state","$window","Authentication","studentResolve"]}();
!function(){"use strict";function StudentsService($resource){return $resource("api/students/:studentId",{studentId:"@_id"},{update:{method:"PUT"}})}angular.module("students").factory("StudentsService",StudentsService),StudentsService.$inject=["$resource"]}();
